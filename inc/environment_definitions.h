...
struct Env {
	...
	//To keep track of the shared variables of this particular env (USEFUL FOR ENV_FREE)
#if USE_KHEAP == 0
	uint32 ENV_MAX_SHARES ; //to be assigned in env_create to [PAGE_SIZE/sizeof(struct SharingVarInfo)]
	struct SharingVarInfo ptr_sharing_variables[PAGE_SIZE/sizeof(struct SharingVarInfo)];
#else
	uint32 ENV_MAX_SHARES;
