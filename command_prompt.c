...
int execute_command(char *command_string)
{
	...
	// Lookup in the commands array and execute the command
		    int command_found = 0, SIZE = 0;
		    int i;
		    char *ptr[NUM_OF_COMMANDS];
		    for (i = 0; i < NUM_OF_COMMANDS; i++)
		    {
		        ...
		        if(strlen(commands[i].name) >= strlen(arguments[0]) &&
		                strncmp(commands[i].name, arguments[0], strlen(arguments[0])) == 0)
		        {
		            ptr[SIZE] = commands[i].name;
		            SIZE++;
		        }
		    }
