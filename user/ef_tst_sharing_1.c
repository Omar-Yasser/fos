...
_main(void)
{
	...
	cprintf("STEP A: checking the creation of shared variables... \n");
	{
		...
		if (x != (int32*)USER_HEAP_START) panic("Returned address is not correct. check the setting of it and/or the updating of the shared_mem_free_address");
		...
		if (z != (int32*)(USER_HEAP_START + 1 * PAGE_SIZE)) panic("Returned address is not correct. check the setting of it and/or the updating of the shared_mem_free_address");
		...
		freeFrames = sys_calculate_free_frames() ;
		y = smalloc("z", 4, 1);
		if (y != (int32*)(USER_HEAP_START + 3 * PAGE_SIZE)) panic("Returned address is not correct. check the setting of it and/or the updating of the shared_mem_free_address");
